...
class YG_EXPORT Style {
 public:
  ...
  FloatOptional resolvedMinDimension(
    ...
    FloatOptional value = pool_.resolveSize(
 ...
 private:
  ...
  // These xxxHandle() methods return raw StyleValueHandles instead of the
  // previously-returned resolved StyleLength. Calc resolution requires viewport
  // dimensions that Style doesn't own, so callers resolve via
  // StyleValuePool::resolveLength() with explicit viewport args.
