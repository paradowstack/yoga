...
class StyleValuePool {
 public:
  ...
  // Fast-path resolution directly from a handle, avoiding construction of
  // intermediate StyleLength/StyleSizeLength objects. For Point/Percent the
  // value is decoded inline; for Calc it delegates to resolveCalc().
 ...
 private:
  ...
  // Packs 4 calc floats (px, percent, vw, vh) into 2 contiguous 64-bit
  // buffer slots (= 4 × 32-bit chunks at indices [N, N+1, N+2, N+3]).
  // getCalc() retrieves them via get64(index) and get64(index + 2).
