...
struct StyleCalcLength {
  ...
  // Downcasts to simpler YGUnit types when possible (e.g. a calc with only a
  // px component becomes YGUnitPoint) so existing consumers that don't know
  // about calc() keep working unchanged.
  explicit constexpr operator YGValue() const {
    if (isUndefined()) {
      return YGValueUndefined;
    }
    if (isPointsOnly()) {
      return YGValue{.value = {.scalar = px_.unwrap()}, .unit = YGUnitPoint};
    }
    if (isPercentOnly()) {
      return YGValue{
          .value = {.scalar = percent_.unwrap()}, .unit = YGUnitPercent};
    }
    return YGValue{
        .value =
            {.calc =
                 {px_.unwrapOrDefault(0.f),
                  percent_.unwrapOrDefault(0.f),
                  vw_.unwrapOrDefault(0.f),
                  vh_.unwrapOrDefault(0.f)}},
        .unit = YGUnitCalc};
  }
